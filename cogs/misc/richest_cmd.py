...
from core.icons import ICON_CROWN, ICON_INFO, ICON_ERROR # Đảm bảo các icon này có trong core/icons.py
...
class RichestCommandCog(commands.Cog, name="Richest Command"):
    def __init__(self, bot: commands.Bot):
        self.bot = bot
        logger.debug("RichestCommandCog initialized.")
    ...
    async def richest(self, ctx: commands.Context):
        ...
        if guild_id not in data or not data[guild_id] or all(key == "config" for key in data[guild_id]):
            logger.info(f"Không có dữ liệu để tìm người giàu nhất cho guild {guild_id}.")
        ...
        if not guild_user_data:
            logger.info(f"Không có user data hợp lệ để tìm người giàu nhất cho guild {guild_id}.")
            await try_send(ctx, content=f"{ICON_INFO} Chưa có ai để xếp hạng trên server này!")
            return
        ...
        if not sorted_users: 
            logger.info(f"Không có user nào sau khi sắp xếp để tìm người giàu nhất cho guild {guild_id}.")
            await try_send(ctx, content=f"{ICON_INFO} Chưa có ai để xếp hạng trên server này!")
            return
        ...
        try:
            ...
            # Hành động hiển thị người giàu nhất có thể coi là INFO nếu bạn muốn log nó vào player_actions.log
            # Hoặc DEBUG nếu chỉ muốn nó trong general_log
            logger.info(f"Người giàu nhất server {ctx.guild.name} (ID: {ctx.guild.id}) là {user_obj.name} (ID: {top_user_id}) với {total_wealth:,} {CURRENCY_SYMBOL}.")
