...
# from core.utils import try_send # Không còn dùng trực tiếp trong file này nữa
...
class HelpSlashCommandCog(commands.Cog, name="Help Slash Command Final"):
    def __init__(self, bot: commands.Bot):
        self.bot = bot
        logger.info(f"HelpSlashCommandCog (Final Version with Logging) initialized.") # Dùng INFO để thấy trên console

    async def _send_general_help_embed(self, interaction: nextcord.Interaction):
        logger.debug(f"Entering _send_general_help_embed for {interaction.user.name}")
        try:
            ...
            logger.debug(f"General help followup sent successfully to {interaction.user.name}.")

        except Exception as e:
            logger.error(f"Lỗi trong _send_general_help_embed:", exc_info=True)
            try:
                ...
                logger.error(f"Không thể gửi tin nhắn lỗi followup cho general help: {followup_e}", exc_info=True)


    async def _send_specific_command_help_embed(self, interaction: nextcord.Interaction, command_name_input: str):
        logger.debug(f"Entering _send_specific_command_help_embed for command: '{command_name_input}' by {interaction.user.name}")
        try:
            prefix = COMMAND_PREFIX
            ...
            if not command_obj:
                logger.warning(f"Command '{cmd_name_to_find_initially}' (and its potential bare map) not found for specific help requested by {interaction.user.name}.")
            ...
            await interaction.followup.send(embed=embed, ephemeral=True)
            logger.debug(f"Specific help followup sent successfully for '{command_obj.name}' to {interaction.user.name}.")
        except Exception as e:
            logger.error(f"Lỗi trong _send_specific_command_help_embed cho lệnh '{command_name_input}':", exc_info=True)
            try:
                if not interaction.is_expired():
                ...
                logger.error(f"Không thể gửi tin nhắn lỗi followup cho specific help: {followup_e}", exc_info=True)
    ...
    async def help_slash_command(self,
        ...
        logger.debug(f"/help slash command invoked by {interaction.user.name} (ID: {interaction.user.id}). Argument 'lệnh': '{command_name}'")
        try:
            if not interaction.response.is_done():
                await interaction.response.defer(ephemeral=True)
                logger.debug(f"Interaction for /help by {interaction.user.name} deferred.")
            else:
                logger.debug(f"Interaction for /help by {interaction.user.name} was already deferred/responded.")
            
            if not command_name:
                logger.debug(f"Calling _send_general_help_embed for {interaction.user.name}...")
                await self._send_general_help_embed(interaction)
            else:
                logger.debug(f"Calling _send_specific_command_help_embed for '{command_name}' by {interaction.user.name}...")
                await self._send_specific_command_help_embed(interaction, command_name)
        except Exception as e:
            logger.critical(f"Lỗi nghiêm trọng không bắt được trong help_slash_command bởi {interaction.user.name}:", exc_info=True)
            try:
                if not interaction.is_expired(): # Kiểm tra trước khi gửi followup cuối cùng
                    # Chỉ gửi followup nếu interaction chưa được phản hồi bằng cách nào đó sau defer và lỗi
                    # Hoặc nếu các hàm con không gửi được followup lỗi của chúng
                    if not interaction.response.is_done(): # Logic này có thể không cần thiết nếu defer luôn chạy trước
                         await interaction.response.send_message(content=f"{ICON_ERROR} Đã có lỗi nghiêm trọng khi xử lý yêu cầu `/help` của bạn.",ephemeral=True)
                    else: # Nếu đã defer (is_done() là True)
                ...
                logger.error(f"Không thể gửi thông báo lỗi cuối cùng cho help_slash_command: {final_followup_e}", exc_info=True)
