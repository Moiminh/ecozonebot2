...
class WorkCommandCog(commands.Cog, name="Work Command"):
    def __init__(self, bot: commands.Bot):
        self.bot = bot
        logger.debug(f"WorkCommandCog initialized for Ecoworld Economy.")
    ...
    async def work(self, ctx: commands.Context):
        ...
        time_left = get_time_left_str(global_profile.get("last_work_global"), WORK_COOLDOWN)
        if time_left:
        ...
        server_data["local_balance"]["earned"] = original_local_earned + earnings
        server_data["xp_local"] = original_xp_local + xp_earned_local
        global_profile["xp_global"] = original_xp_global + xp_earned_global
        global_profile["last_work_global"] = datetime.now().timestamp()
