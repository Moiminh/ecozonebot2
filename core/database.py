...
def get_or_create_user_local_data(global_user_profile: Dict[str, Any], guild_id: int) -> Dict[str, Any]:
    ...
    if guild_id_str not in server_data_pool:
        ...
        server_data_pool[guild_id_str]["survival_stats"] = DEFAULT_USER_LOCAL_DATA["survival_stats"].copy()
    ...
    if "earned" not in balance: balance["earned"] = 0; data_changed = True
    if "adadd" not in balance: balance["adadd"] = 0; data_changed = True
...
def add_moderator_id(user_id: int, file_path: str = 'moderators.json') -> bool:
    """Thêm một ID moderator và lưu lại file."""
    ids = load_moderator_ids(file_path)
    if user_id not in ids:
        ids.append(user_id)
        save_moderator_ids(ids, file_path)
        return True
    return False

def remove_moderator_id(user_id: int, file_path: str = 'moderators.json') -> bool:
    """Xóa một ID moderator và lưu lại file."""
    ids = load_moderator_ids(file_path)
    if user_id in ids:
        ids.remove(user_id)
        save_moderator_ids(ids, file_path)
